parallelWindowNPO_v3 <- function() {
  # Built from parallelWindowNPO_v2.R
  #
  # v3.0           Receives events sequentially and sends them to a EventsBufferFutures.
  # June 22, 2021
  # Mark R. Bower
  # Yale University
  library(future)
  ...
  # Make an EventsBufferFutures.
  dbp <- databaseProvider(user=args$user,vault_user=args$vault_user,vault_key=args$vault_key,host=args$host,dbname=args$dbname)
  ebf <- eventsBufferFutures( size=20000, NPO:::NPO_futures(dbp=dbp) )
  
  iter_t <- itertools::ihasNext( iterators::iter( T ) )
  while ( itertools::hasNext( iter_T ) ) {
    ebf$add( nextElem( iter_T ) )
  }
  ebf$flush()
