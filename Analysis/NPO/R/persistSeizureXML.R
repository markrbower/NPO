persistSeizureXML <- function( dbuser='root', host='localhost', password='', dbName, dbTable, fields, xmlFile, subject, limit ) {
  # Assumes that the data are written: start, stop, label
  #
  # Usage:
  # persistSeizureXML( db, 'epochs', list('subject','session', label','start','stop'), '/Users/markrbower/Documents/Data/NV/NVC1001_23_002_2', "12 23 13 25 13 14 12 15" )
...
#  dat <- xml2::read_xml( paste0("/Users/markrbower/Documents/Data/NV/NVC1001_",subject,"_2/annotations.xml") )
  dat <- xml2::read_xml( paste0("/Volumes/Data/NV/NVC1001_",subject,"_2/annotations.xml") )
  nodes<-xml2::xml_find_all(dat, ".//annotation")
  nodes<-nodes[xml_attr(nodes, "description")=="Seizure_CCS"]
  starts <- xml2::xml_attr(nodes, "startOffsetUsecs")
  stops  <- xml2::xml_attr(nodes, "endOffsetUsecs")
  data <- data.frame( starts=starts, stops=stops )

#  dib <- topconnect::databaseInsertBuffer( dbConn, dbTable, fields, 100 )
  dib <- topconnect::databaseInsertBuffer( dbName, dbTable, fields, limit, updates=NULL, dbuser='root', host='localhost', password='' )
