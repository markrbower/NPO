  ...
  if ( length(messages) > 0 ) {
    ...
    if ( itertools::hasNext(iter_messages) ) {
      ...
      while ( itertools::hasNext(iter_messages) ) {
        ...
        p <- mcparallel( NPO:::computeCCfwd( CW, cc_threshold, ed_threshold, cntMessage-1, message, voltage, data$time[dataIdxBoth], data$voltage[dataIdxBoth], wait=TRUE ) )
        results <- mccollect( list(p), wait=TRUE )
        
        # ... with this step waiting for a Future
        CC <-  rbind( CC_next, results[["CC"]] )
        
### This part can be done in parallel. It doesn't mattere when it finishes.
### As long as the output data structure is sorted at somee point.
        p <- mcparallel( ... do the graph work ... )
