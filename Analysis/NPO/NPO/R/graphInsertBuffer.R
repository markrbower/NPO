...
#  gib <- graphInsertBuffer( parameters, 200, .9, 1.5, dib, 'tmp.xml', 20 ); gib$insert(e1); gib$insert(e2); gib$insert(e3); gib$insert(e4); gib$insert(e5)
#  grph <- gib$makeGraph()
  # print( grph )
  # plot( grph )
...
#  print( graph_filename )
  ...
  mask <- parameters$computation_mask
...
#  insert <- function( event, utc, state, dib ) { # event is a list of time and waveform-as-string
  insert <- function( event ) { # event is a list of time and waveform-as-string
    ...
    if ( !( names(event) %in% names(events) ) ) {
      ...
      if ( length(td) > 0 & max(td) > blackout ) { # then there is some link to make
        # print( "In" )
...
#            }
  ...
  processSubgraph <- function( Nlimit, state, append=TRUE ) {
    # Returns an array of subgraphs for each community.
...
#          print( str_incident )
          ...
          if ( !is.null(dib) ) {
            dib$insert( list(subject=state$subject, session=state$session, channel=state$channel, seizureUsed=state$seizureUsed, time=as.numeric(gr_label), waveform=waveform, peak=peak, energy=energy, clusterid=lookup_table[gr_id], incident=str_incident, weights=str_weights ) )
          }
        }
      }
    }
...
#    print( paste0( utc, ' ', N, ' ', L ) )
    if ( N >= dib$updateNumber() ) {
      state <- processSubgraph( (utc-persistInDbWindow), state, TRUE )
    }
  }
